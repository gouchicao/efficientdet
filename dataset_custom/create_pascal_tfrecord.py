...
from dataset_custom import tfrecord_util
...
def dict_to_tf_example(data,
  ...
  if 'object' in data:
    for obj in data['object']:
      if not obj['name'] in pascal_label_map_dict:
        continue
...
def main(_):
  ...
  for year in years:
    ...
    examples_list = []
    for class_name in pascal_label_map_dict:
      if class_name == 'background':
        continue
      examples_path = os.path.join(data_dir, year, 'ImageSets', 'Main', class_name + '_' + FLAGS.set + '.txt')
